...
attendu : c'est la technique des tests.  Les tests «boîte noire» consistent à
tester en ayant à disposition uniquement les spécifications des modules à
plusieurs échelles. L'application est découpée en «boîtes» à plusieurs échelles
(par exemple : logiciel, module, classe, méthode). Au contraire, les tests dits
«boîte blanche» sont écrits en ayant à disposition l'implémentation. Cela
permet par exemple de s'assurer que chaque chemin d'exécution est emprunté.
...
extensions non standard peuvent être utilisées. Par exemple, le compilateur GCC
ajoute des extensions pour personaliser finement le code émis.
