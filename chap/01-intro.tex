...
\section{Séparation entre espace noyau et espace utilisateur}
\label{sec:sep-esp}
...
Aujourd'hui, la réussite de missions dépend de logiciels dont la taille est de
plus en plus grande. Ainsi en cas de fautes dans ce genre de logiciel, on peut
se retrouver face à de grands impacts économiques, voire risquer des vies
humaines. On comprend bien que les phases de vérification et de certification
sont au cœur du cycle de vie des logiciels avioniques. A titre d'exemple,
l'échec du premier vol d'Ariane 5 aurait certainement pu être évité si le
logiciel de contrôle de vol avait été vérifié plus
...
\def\appx#1{
\draw [yshift=#1,patB,scale around={1.3:(3,3)}]
                (1,1) -- (1,2)
             -- (2,3) -- (3,3)
             -- (3,1) -- cycle
}
...
\def\appximp#1{
\draw [xshift=180,yshift=#1,patB,scale around={1.3:(3,3)}]
                (1,1) -- (1,2)
             -- (4,5)
             -- (4,1) -- cycle
}
...
\def\semok#1{
\draw [xshift=#1,yshift=180,patC,scale=0.8] plot[smooth cycle]
      coordinates{(1,1) (1,2) (2,3) (3,3) (3,1)}
}

\def\semko#1{
\draw [xshift=#1,patC,scale=0.8] plot[smooth cycle]
      coordinates{(1,1) (1,2) (2,3) (3,3) (4.5,5) (3,1)}
}

\def\enssurR#1#2{
\draw [xshift=#1,yshift=#2]
                 (0, 0) -- ++(0, 2)
            -- ++(1, 2) -- ++(3,-1)
            -- ++(1,-3) --    cycle;
}
...
\subbottom[]{
\label{fig:appx-proof}
\begin{tikzpicture}[scale=0.7]
\clip  (-0.5,5.5) rectangle (5.5,12);
...
\subbottom[]{
\label{fig:appx-unsound}
\begin{tikzpicture}[scale=0.7]
\clip  (-0.5,-0.5) rectangle (5.5,6);
...
\subbottom[]{
\label{fig:appx-warn-ok}
\begin{tikzpicture}[scale=0.7]
\clip  (6,-0.5) rectangle (12,6);
...
des états effectifs du programme, noté par des points, est approximé par
l'ensemble en gris.}
...
% TODO[E] cela arrive un peu tard, en fire un mot en début de sec ou fin page 7
...
disponibles sous license libre sur~\link{penjili}. L'analyseur statique Penjili,
...
% TODO[E] dire c'est pour traiter quoi
...
\changed{mention des bitmasks et de ptrace}
...
% TODO[S] en les adaptant -> pas clair. en quoi npk permet cela?
...
%TODO[E] problèmes d'addressage: préciser
