...
l'appel à une $\mathrm{cast}_{t_1,t_2}$, on ajoute alors un ``trou'' dans le
système de types. Cette fonction devrait en effet être typée $(t_1) → t_2$,
autrement dit le type ``maudit'' $α → β$ de \texttt{Obj.magic} en OCaml ou
...
analayse sont corrects de ce point de vue, on peut désactiver ces vérifications
...
% TODO citer the linux memory manager ?
...
Une autre limitation est que seules structures simples sont supportées. Les
structures récursives, contenant un pointeur vers un objet du même type
structure, ne sont pas typables. La raison est syntaxique : l'accès à un
champ est noté $lv.l_S$ afin d'avoir à disposition le type complet de la
structure.
...
% TODO est-ce vrai ? Y'a-t-il des cas où les forward decl sont nécessaires?
