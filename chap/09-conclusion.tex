...
\paragraph{Les fonctions \texttt{setjmp} et \texttt{longjmp}}

% TODO goto
% TODO goto c7

Le langage C propose deux fonctions qui permettent de sauvegarder un état
d'exécution et d'y sauter: ce sont respectivement les fonctions \texttt{setjmp}
et \texttt{longjmp}. Elles sont très puissantes puisqu'elles permettent
d'exprimer n'importe quel flot de contrôle, à la manière d'une continuation.
...
polymorphisme de rangée (\emph{row polymorphism}): plutôt que de cacher les
champs inconnus derrière le sous-typage, on en fait une variable $ρ$ qui pourra
s'unifier avec d'autres champs. Par exemple, partant de $x.l ← 0$ on déduit que
$x.l$ a pour type $\tInt$ et que $x$ a pour type $\{ l : \tInt ; ρ \} $.
En pratique, l'implantation décrite dans le chapitre~\ref{cha:implem} utilise
une technique similaire pour inférer les types des structures à partir du code
\newspeak.
