...
analyse ne comportent pas de telles erreurs de programmation, on peut désactiver
ces vérifications et le reste des propriétés est toujours valable.
...
% TODO attention, on peut finir avec une seule région (pire des cas)
...
d'utiliser l'outil Coccinelle pour convertir du code vers ce
style~\cite{lctes11}.
% TODO pas clair
On peut imaginer qu'il est possible de l'utiliser pour
...
très puissantes puisqu'elles permettent d'exprimer n'importe quelle structure de
contrôle, à la manière d'une continuation.

% TODO attention pour une continuation il te faut la pile en plus
...
% TODO il faut probablement indiquer ici pourquoi tu as préféré ne pas le faire
% pour le noyau traité
