...
\subsection{Introduction à l'assembleur Intel}
...
l'adresse de l'instruction courante. \eflags est un registre de "fanions"
(\emph{flags}), qui sont des indicateurs positionnés selon le résultat d'un
calcul précédent.

Les calculs sont décrits sous forme d'une suite d'instructions. Chaque
instruction est composée d'une mnémonique et d'une liste d'arguments. Les
...
\item
  ...
  adresse effective. C'est adresse effective peut être exprimée de
  plusieurs manières :
  \begin{itemize}
  \item
...
\caption[Cadres de pile]{
Cadres de pile.

Pour appeler \texttt{g(4,2)}, \texttt{f} empile les arguments de droite à
gauche. L'instruction \asminstr{call g} empile l'adresse suivante sur la pile
puis saute au début de \texttt{g}.
...
fixe dans la pile (comme l'adresse de retour). Ce mécanisme est décrit dans la
figure~\ref{fig:stackframe}.
