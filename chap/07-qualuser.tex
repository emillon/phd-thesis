...
\section{Provenance des pointeurs}

\subsection{Éditions et ajouts}

La sémantique d'évaluation du langage n'est pas modifiée. En revanche, on
modifie légèrement le système de types (figure~\ref{fig:qualif-changes}) afin
...
\begin{align*}
i \gramisa …
\\ \gramor & \iTaint{x} & \textrm{Teintage d'une valeur}
\end{align*}
}
...
\begin{align*}
ms \gramisa & …
\\  \gramor & Ω_{taint} & \textrm{Erreur de taintage}
\end{align*}
}
...
Puisque la définition de $φ$ a été changée, il est aussi nécessaire de redéfinir
la lentille $Φ$ utilisée dans l'évaluation des expressions
