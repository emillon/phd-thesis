...
\begin{align*}
ms \gramisa & …
\\  \gramor & \serr{taint} & \textrm{Erreur de souillure}
\end{align*}
}
...
$\serr{array}$ ou $\serr{ptr}$, violant ainsi le théorème~\ref{thm:progres}.
...
Pour typer les expressions composées (structures et tableaux), il faut remarquer
que si l'utilisateur contrôle l'adresse d'une expression composée, alors il
contrôle tous ces champs pointeurs. En quelque sorte, il faut s'assurer que si
$Γ ⊢_q \& \{l_1:t_1;…;l_n:t_n\}$, alors aucun $t_i$ n'est de la forme
$t~\qKernel~*$.

Pour ce faire, on commence par ordonner les qualificateurs en posant $\qKernel
\sqsubseteq \qUser$, et on définit une règle de typage telle que le
qualificateur associé à l'adresse d'une expression soit l'union des
qualificateurs présents dans son type.
...
\begin{definition}[Union de qualificateurs]
  ...
  (cela est équivalent à définir un treillis
  $\{\qKernel ; \qUser \}$
  avec
  $\qKernel \sqsubseteq \qUser$
  )

\end{definition}

L'opérateur $Q_\&$ défini ci-après permet d'exprimer "le plus grand
qualificateur présent dans un type".
...
Alors la règle pour typer $\&lv$ devient :
