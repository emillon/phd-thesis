...
expressions avec les valeurs qui leur seront associées. On peut voir cela comme
une forme d'analyse de flot: si peu de constructions permettent de créer des
valeurs d'un type $t$, alors toutes les valeurs de type $t$ proviennent de ces
\enquote{sources}.

On se propose ici d'enrichir le système de types de plusieurs extensions
permettant d'explorer cette idée, en ajoutant du \enquote{sens} dans les types
de données des programmes. Ces extensions permettront de détecter des erreurs de
programmation communes, appuyées sur des exemples réels.

On commence par un exemple introductif permettant distinguer plusieurs
utilisations de types entiers, selon s'ils sont utilisés comme entiers
arithmétiques ou ensemble de bits. Cela permet de détecter une erreur qui
consiste à mélanger les opérateurs logiques et bit à bit.

Ensuite, on étend de manière indépendante le système de types, cette fois au
niveau des pointeurs. Plus précisément, dans le contexte de systèmes
...
un \texttt{unsigned int}) encode de la même manière les permissions que doit
avoir le fichier créé, le cas échéant.
...
% TODO[E] Qui se termine par un \0? (le litt)
...
valeurs \tInt et le type statique \tBits (cela signifie qu'une valeur de type
\tBits est représentée par un \tInt en mémoire,
...
% TODO[E] cela veut dire qu'on peut additionner un \tBits et un \tInt pour obtenir
% un \tInt ?
...
Cela conclut cet exemple préliminaire, qui permet de voir en quoi \langname est
