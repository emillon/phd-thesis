...
\begin{align*}
  ...
  & \indent \uPut{\& (p.\texttt{major}_S)}{\& x} ; & \mathrm{où} & & S = \{~& \texttt{major} : \tInt ; \\
  & \indent \uPut{\& (p.\texttt{minor}_S)}{\& y} ; &             & &        & \texttt{minor} : \tInt ; \\
  & \indent \uPut{\& (p.\texttt{patch}_S)}{\& z}   &             & &        & \texttt{patch} : \tInt   \\
  & \}                                             &             & &     \}
\end{align*}
...
\caption{Ajouts liés aux qualificateurs de types (par rapport à la
