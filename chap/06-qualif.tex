...
alors écrit comme dans la figure~\ref{fig:ex-sys-getver}.
...
\begin{align*}
  ...
  & \indent \textsc{Decl}~x=3~\textsc{in}          \\
  & \indent \textsc{Decl}~y=14~\textsc{in}         \\
  & \indent \textsc{Decl}~z=15~\textsc{in}         \\
  & \indent \uPut{\& (p.\texttt{major}_S)}{\& x} ; \\
  & \indent \uPut{\& (p.\texttt{minor}_S)}{\& y} ; \\
  & \indent \uPut{\& (p.\texttt{patch}_S)}{\& z}   \\
  & \}
\end{align*}
\end{minipage}
\begin{minipage}{2cm}
...
\caption{Exemple d'implantation d'un appel système qui remplit une structure par
pointeur}
...
l'espace utilisateur (c'est-à-dire, qu'il est da la forme $\vTainted{φ}$.
