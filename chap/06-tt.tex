...
\item
  si $Γ ⊢ e (e_1, …, e_n) : t$, il existe $(t_1, …, t_n)$
  tels que
...
\begin{lemma}[Formes canoniques]
  ...
  On peut déterminer la forme syntaxique d'une valeur d'après son type :
...
\begin{lemma}[Permutation]
  Si $Γ ⊢ e : t$ et $Γ' = σ(Γ)$, alors $Γ' ⊢ e : t$.
\end{lemma}

\begin{lemma}[Affaiblissement]
  Si $Γ ⊢ e : t$ et $x ∉ dom(Γ)$, alors $Γ, x : t' ⊢ e : t$.
\end{lemma}

\begin{lemma}[Substitution]
  Si $Γ, x : t' ⊢ e : t$ et $Γ ⊢ e' : t'$, alors $Γ ⊢ e [e'/x] : t$.
\end{lemma}
...
\item noter les flottants et fonctions différemment
\end{itemize}
