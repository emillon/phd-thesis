...
\item $e = v$. On applique alors l'hypothèse de récurrence à $i$ sous
    ...
    % TODO prouver la mcomp

    \begin{itemize}
    \item $i = \iPass$. Dans ce cas la règle \textsc{Decl-Pass} s'applique.
        Il reste à prouver que $\mcomp{Γ'}{m''}$ où $m'' = \cCleanup{m'-x}$.
        % TODO ou que Γ plutôt
...
\]
    ...
    \item $\mm{m'}{i}{m''}{i'}$. Par \textsc{Decl-Ctx} il vient
       ...
       $v' = m''[(0, x)]_A$
       et
       $m''' = m'' - x$. Il ne reste qu'à prouver que $\mcomp{Γ}{m'''}$. % TODO
...
Alors $\mcomp{Γ'}{m_1}$. % TODO

On applique alors l'hypothèse de récurrence à $Γ'$, $m_1$ et $i$.
% TODO pourquoi a-t'on Γ' ⊢ i ?

\begin{itemize}
\item $i = \iPass$. Alors on peut appliquer \textsc{Exp-Return-Unit}.
    Il reste à prouver que $\mcomp{Γ}{m'}$ où $m' = \cCleanup{m_1}$
...
\item $\mm{m_1}{i}{m_2}{i'}$.
    ...
    Il reste à montrer que $\mcomp{Γ}{m_3}$. % TODO
