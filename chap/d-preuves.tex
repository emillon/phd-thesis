...
valeur gauche. De même la règle \textsc{Trans} ne pose pas de problème, il
suffit d'appliquer l'hypothèse de récurrence à ses prémisses.
...
Ce cas repose sur un lemme d'inversion des termes typés sous un contexte
(admis).

Puisque $Γ ⊢ \ctxSub{C}{lv} : t$, il existe $Γ'$ et $t'$ tels que:
...
On applique l'hypothèse de récurrence à la prémisse de \textsc{Ctx},
c'est-à-dire $\mm{m}{lv}{m'}{lv'}$. Il vient que $Γ' ⊢ lv' : t'$ et
$\mcomp{Γ'}{\cCleanup{m'}}$. D'après le précédent lemme d'inversion on en déduit
que $Γ ⊢ \ctxSub{C}{lv'} : t$.

On admet que $\mcomp{Γ}{\cCleanup{m'}}$.
...
\paragraph{\textsc{Ctx}:} % {{{
Idem que dans le cas des valeurs gauches.
% }}}
