...
$C = \iReturn{\ctxEmpty}$\footnote{Les contextes sont des objets purement
syntaxiques: on peut les appliquer entre instructions et expressions
indifféremment}, on conclut que
\linebreak
...
\begin{itemize}
\item $i = \iReturn{v}$. Alors on applique \textsc{Exp-Call-Return}.
...
\item $\mm{m_1}{i}{m_2}{i'}$. Alors on peut appliquer \textsc{Exp-Call-Ctx}.
...
Il n'est pas nécessaire de montrer la compatibilité de $m'$ car la mémoire n'est
pas modifiée. De plus, les prémisses de ces règles ont la forme $φ$ , donc le
...
Enfin par \textsc{If} il vient $Γ ⊢ \iIf{e}{i ; \iWhile{e}{i}}{\iPass}$.
