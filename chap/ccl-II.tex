...
syntaxe et sa sémantique dans le chapitre~\ref{cha:typbase}.
Une des particularités de cette sémantique est l'utilisation de lentilles pour
...
alors encodés comme des actions monadiques qui représentent les modifications de
la mémoire. Un des avantages de cette solution est qu'elle est très extensible.
Par exemple, la propagation des erreurs, ou l'ajout de continuations
(c'est-à-dire le support des fonctions \texttt{setjmp} et \texttt{longjmp})
peuvent facilement être exprimées dans un formalisme monadique.
...
théorème de préservation (théorème~\ref{thm:préservation},
