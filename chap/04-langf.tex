Dans ce chapitre, on décrit le cœur de notre travail: le langage \langname, sur
...
\item $\serr{ptr}$ correspond à une manipulation ou un déréférencement de
pointeur invalide. Par exemple, déréférencer le pointeur nul ou appliquer
l'arithmétique en dehors d'un tableau provoque cette erreur.
...
\item $\serr{typ}$ est levée lorsque une opération est appellée sur des types de
données incompatibles. Par exemple, appliquer un «$-$» unaire sur une valeur
pointeur.
...
\begin{align*}
      \mathrm{Cleanup}((s, g)) &= (s', g') \\
                 \mbox{où } s' &= (\cL{|s|}{s_1}, …, \cL{|s|}{s_|s|}) \\
                            g' &= \cL{|s|}{g} \\
\cL{p}{(x_1 ↦ v_1, …, x_n ↦ v_n)} &= (x_1 ↦ \cV{p}{v_1}, …, x_n ↦ \cV{p}{v_n}) \\
...
Sans cette règle, examinons le programme suivant de la
...
avec ce couple (variable, valeur) (figure~\ref{fig:eval-toplevel}). Notons que
ces évaluations se font à grands pas.
