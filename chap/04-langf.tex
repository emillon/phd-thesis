...
Un des problèmes classiques dans les langages impératifs est de distinguer les
fonctions (qui retournent une valeur) et les procédures (qui n'en retournent
pas). La solution choisie par C est de marquer les procédures comme retournant
un ``faux'' type \texttt{void}. Mais c'est uniquement syntaxique : il n'est pas
possible de manipuler cette valeur de retour de type \texttt{void}.
...
un $n$-uplet à 0 composantes.}, et donc le type sera noté \tUnit.
...
\begin{figure}
  ...
  \gramlr{Constantes}{
    \begin{align*}
      ...
      \\ \gramor & \eUnit & \textrm{Valeur unité}
    \end{align*}
  }
...
\begin{figure}%{{{
  ...
  \gramlr{État d'interpréteur}{
    \begin{align*}
      Ξ \gramisa & \msi{m}{e} & \textrm{Expression, mémoire}
      \\ \gramor & \msi{m}{i} & \textrm{Instruction, mémoire}
      \\ \gramor & Ω          & \textrm{Erreur}
    \end{align*}
  }

  \gramlr{Erreur}{
    \begin{align*}
      Ω \gramisa & Ω_{array}  & \textrm{Dépassement de tableau}
      \\ \gramor & Ω_{ptr}    & \textrm{Erreur de pointeur}
      \\ \gramor & Ω_{div}    & \textrm{Division par zéro}
    \end{align*}
  }
...
% TODO
% TODO déplacer en semrule
% TODO mettre un e (ou v) ::= Ω
...
\begin{mathpar}
  ...
  % TODO Règle à vérifier: ça suffit?
  \irule{Eval-Err}
    { }
    { \msi{m}{\ctxSub{C}{Ω}} → Ω }
\end{mathpar}
