...
\textsc{ET-Exp}, qui a comme prémisse $\mm{m_3}{f(\&x)}{m_4}{v}$.
...
Donc d'après \textsc{Exp-Call-Return} (car on a $m_3''' = \cCleanup{m_3'''}$ et
$ \widehat{\eUnit} = \cV{|m_3'''|}{\widehat{\eUnit}} $):

\[
    \mm{m_3}{f(\&x)}{m_3'''}{ \widehat{\eUnit} }
\]

En posant $m_4 = m_3'''$, on a $\ph{m_3}{f(\&x)}{m_4}$.

Donc pour conclure (grâce à \textsc{Prog}), on a
$\phprog{[p_1,p_2,p_3,p_4]}{m_4}$.
