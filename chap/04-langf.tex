...
Ce langage servira de support au système de types décrit dans le
...
\begin{align*}
\gramdef[3cm]{Arbres binaires}{a} { \textsc{F}              }{Feuille}
...
d'identificateurs $\sId$. % TODO inutile?
...
% TODO INDIQUER LES AUTRES CHOIX
...
% TODO il faut introduire pourquoi tu les utilises
...
$\mathrm{put}_Q (4::2::[], 3::6::1::5::[]) = 3::4::2::[]$.
...
% TODO mettre la syntaxe directe et intégrer ça a leur place
...
c'est uniquement syntaxique: il n'est pas possible de manipuler de valeur de
type \texttt{void}.
...
valeur d'un type à un seul élément noté \eUnit\footnote{Cette notation évoque
...
la seule forme --- rudimentaire --- de généricité qu'offre le langage). En
\langname, la valeur $\eUnit$ est une valeur comme les autres, et on peut
constuire un pointeur de type $\tUnit *$ qui n'aura pas de signification
particulière: c'est seulement un pointeur vers une valeur de type \tUnit.
...
\paragraph{Types de données:} Entiers machine, flottants, pointeurs et types
composés (structures et tableaux) composés de ceux-ci.
...
c'est-à-dire qui ne sont pas des fermetures. Même si les fonctions locales sont
possibles, il n'est pas possible d'accéder aux variables de la portée entourante
depuis la fonction intérieure.
...
de la fonction (liste de paramètres, de variables locales et corps). Remarquons
que contrairement à certains langages, l'environnement n'est pas capturé (il n'y
a pas de clôture lexicale).
...
\item
  ...
  égales si elles ont la même ``forme'' (même taille pour les tableaux, et même
...
\[
  \cLookup{(s, g)}{x} =
        \begin{cases}
            (|s|, x) & \mbox{ si } |s| > 0
                       \mbox{ et } (x ↦ v) ∈ s_1 \\ % TODO le dernier
            x        & \mbox{ si } (x ↦ v) ∈ g \\
            \serr{var} & \mbox{ sinon }
        \end{cases}
\]
...
% TODO je ne comprends pas la définition
% TODO est-ce toujours le cadre local?h
...
\label{subsec:acces-phi}
  ...
  \begin{align*}
    Φ(φ.l)  &= Φ(φ) \ggg F(l) \\ % TODO d'où sortent ils? - rappel
    Φ(φ[i]) &= Φ(φ) \ggg T(i) \\
  \end{align*}
...
% TODO figure cleanup fausse depuis les derefs changés
...
d'évaluation de $\eFun{a}{i} (v_1,…,v_n)$, donc un contexte
$C \gramisa \eFun{a}{\ctxEmpty} (v_1,…,v_n)$ n'est pas à considérer. Il
