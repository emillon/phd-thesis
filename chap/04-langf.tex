...
% TODO[E] un petit rappel du lien entre les 2 *speak?
...
% TODO[E] 2 dernières lignes = à préciser
...
\paragraph{Remarques}
\added{Les 2 paragraphes suivants}
...
Dans le cas où la valeur référencée n'a pas la forme $m[φ']$, aucune règle ne
...
Ce programme est constitué des phrases $p_1, p_2, p_3$ et $p_4$. D'après
\textsc{Prog}, l'évaluer va revenir à évaluer à la suite ces 4 phrases.
...
\paragraph{Déclaration de \texttt{main}}

Encore une fois, on définit une fonction. Ce cas est similaire au précédent et
$\ph{m_2}{main = \eFun{}{f(\&x);\iReturn{\eUnit}}}{m_3}$ où:

\[m_3 = ([~],
[main↦\widehat{\eFun{}{f(\&x);\iReturn{\eUnit}}}
;f↦\widehat{\eFun{q}{*q←1;\iReturn{\eUnit}}}
;x↦\widehat{0}])\]

\paragraph{Appel de \texttt{main}}

Ici, on évalue une expression pour ses effets de bords. La règle à appliquer est
\textsc{ET-Exp}, qui a comme prémisse $\mm{m_3}{main()}{m_4}{v}$.
...
\[
    \mm{m_3}{f}{m_3}{\widehat{\eFun{q}{*q←1;\iReturn{\eUnit}}}}
\]

On en déduit que:

\[
    \mm{m_3}{f(\&x)}{m_3}{\widehat{\eFun{q}{*q←1;\iReturn{\eUnit}}}(\&x)}
\]
...
\[
    \mm{m_3}{f(\&x)}{m_3}{\widehat{\eFun{q}{*q←1;\iReturn{\eUnit}}}(\widehat{\&}(x))}
\]

La fonction et son argument sont évalués, donc on peut appliquer
\textsc{Exp-Call-Ctx}. En posant $m_3' = \cPush{m_3}{(q ↦ \widehat{\&}(x))}$, le
but est de trouver $m_3''$ et $v$ tels que:

\[
    \mm{m_3'}{*q←1;\iReturn{\eUnit}}{m_3''}{\iReturn{v}}
\]
...
Alors, d'après \textsc{Seq}, $\mm{m_3'}{*q←1}{m_3''}{\iReturn{()}}$.
Avec \textsc{Exp-Cst} sous $C = \iReturn{\ctxEmpty}$, on a donc
$\mm{m_3'}{*q←1}{m_3''}{\iReturn{\widehat{()}}}$.

On peut enfin appliquer \textsc{Exp-Call-Ctx} pour en déduire que:

\[
    \mm{m_3}{f(\&x)}{m_3'''}{ \eFun{q}{\iReturn{\widehat{\eUnit}}}(\widehat{\&}(x)) }
\]
...
%TODO[E] il faut effectivement présenter safespeak mais cela donne un chapitre
%de 25 pages où on se perd un peu (en particulier à la fin)
% peut être finir avec un exemple + complet
