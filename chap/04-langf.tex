...
\begin{definition}[Manipulations de pile]
  ...
  % TODO notation pas cohérente avec la sémantique (flèches talonnées)
...
\begin{definition}[Accès par champ]
  ...
  \begin{align*}
    \eStruct{ l_1 : v_1; … ; l_n : v_n }{[l]}_L =
          & v_i \mbox{ si } ∃i∈[1;n], l=l_i\\
    \eStruct{ l_1 : v_1; … ; l_n : v_n }{[l ← v]}_L = &
        \eStruct{ l_1 : v_1 \\
       & ; … \\
       & ; l_{p-1} : v_{p-1} \\
       & ; l_p : v \\
    ...
    \eStruct{ l_1 : v_1; … ; l_n : v_n }{[l]}_L = & \serr{field} \mbox{ sinon} \\
    \eStruct{ l_1 : v_1; … ; l_n : v_n }{[l ← v]}_L = & \serr{field} \mbox{ sinon}
  \end{align*}
...
\begin{definition}[Accès par indice]
  ...
  \begin{align*}
    \eArray{ v_1 ; … ; v_n } {[i]}_I   = & v_{i+1} \textrm{ si } i ∈ [0;n-1] \\
    \eArray{ v_1 ; … ; v_n } {[i]}_I   = & \serr{array} \textrm{ sinon} \\
    \eArray{ v_1 ; … ; v_n } {[i←v]}_I = & \eArray{ v'_1 ; … ; v'_n } \textrm{ si } i ∈ [0;n-1] \\
                      \mbox{ où } & \begin{cases}
                                      v'_i = v \\
                                      ∀j≠i, v'_j = v_j \\
                                    \end{cases} \\
    \eArray{ v_1 ; … ; v_n } {[i←v]}_I = & \serr{array} \textrm{ sinon} \\
  \end{align*}
