...
On considère en fait la cloture transitive de cette relation. Cela revient à
ajouter une règle :
...
\label{fig:stackops-extend}
  \begin{tikzpicture}
    ...
    ;
    \draw [->] (5.2, -2) -- node[auto] {$\mathrm{Extend}((\cdot, y, 3))$} ++(1.6, 0);
  \end{tikzpicture}
  }
...
m {[(n, x)]}_A     & = \begin{cases}
                           \lensGet{ℒ_{n,x}}{s} & \mbox{ si $1 ≤ n ≤ |s|$} \\
                           \serr{var}          & \mbox{ sinon}
                       \end{cases} \\
m {[(n, x) ← v]}_A & = \begin{cases}
                            (\lensPut{ℒ_{n,x}}{v}{s}, g) & \mbox{ si $1 ≤ n ≤ |s|$} \\
                            \serr{var}          & \mbox{ sinon}
                       \end{cases} \\
\end{align*}
...
\begin{align*}
  ...
  \ctxSub{\eStruct{ l_1:v_1 ; …
     ; l_i:C ; … ; l_n:e_n }}{e_0} = & \eStruct{ l_1:v_1 ; … ; l_i:\ctxSub{C}{e_0} ; … ; l_n:e_n } \\
  \ctxSub{\eArray{ v_1 ; … ; C ; … ; e_n }}{e_0} = & \eArray{ v_1 ; … ; \ctxSub{C}{e_0} ; … ; e_n }    \\
...
\ctxSub{\ctxSet{C}{e}}{e_0} = & \ctxSet{\ctxSub{C}{e_0}}{e} \\
\ctxSub{\ctxSet{φ}{C}}{e_0} = & \ctxSet{φ}{\ctxSub{C}{e_0}} \\
\ctxSub{\eStruct{ l_1:v_1 ; … ; l_i:C ; … ; l_n:e_n }}{e_0} = & \eStruct{ l_1:v_1 ; … ; l_i:\ctxSub{C}{e_0} ; … ; l_n:e_n } \\
\ctxSub{\eArray{ v_1 ; … ; C ; … ; e_n }}{e_0} = & \eArray{ v_1 ; … ; \ctxSub{C}{e_0} ; … ; e_n } \\
\ctxSub{C (e_1, …, e_n)}{e_0} = & \ctxSub{C}{e_0} (e_1, …, e_n) \\
\ctxSub{f (v_1, …, C, …, e_n)}{e_0} = & f (v_1, …, \ctxSub{C}{e_0}, …, e_n) \\
\\
...
Le résultat est une expression $e'$, c'est-à-dire que nous présentons une
sémantique à petits pas. \emph{In fine}, cela permet d'évaluer $e$ en une valeur
$v$. Cette évaluation est notée:
...
De même, les left-values sont réduites à petits pas. Chaque pas transforme $lv$
en $lv'$, selon un pas qui peut modifier la mémoire (à travers l'évaluation des
expressions d'indices). Une left-value complètement évaluée est un ``chemin''
$φ$. On note:
...
% TODO expliquer pourquoi Ctx n'est pas possible
...
Enfin, si un ``\emph{return}'' apparaît dans une séquence, on peut supprimer la suite:
...
\begin{mathpar} % TODO la syntaxe est ancienne
  f (a, b) (t = 0) \{
    \iWhile{b ≠ 0}{
      \exinnerif{}
    };
