...
\begin{definition}[Manipulations de pile]
  ...
  L'opération inverse de $\phxx\cExtend$ sera simplement notée «$-$» : $m - x$,
  par exemple.
...
Sans ces opérations, examinons le programme suivant de la
...
Dans le cas où on transmet une instruction terminale ($i = \iPass$ ou $i =
\iReturn{v}$), il faut alors supprimer les références aux variables
qui ont disparu grâce aux opérateurs $\phx\cCleanup$ et $\phxx\cV$.
