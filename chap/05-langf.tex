...
\gramlr{Expression}{
\begin{align*}
  e \gramisa & t     & \textrm{Terme}
  \\ \gramor & e + t & \textrm{Addition}
  \\ \gramor & e - t & \textrm{Soustraction}
\end{align*}
}

\gramlr{Terme}{
\begin{align*}
t \gramisa & f     & \textrm{Facteur}
\\ \gramor & t * f & \textrm{Multiplication}
\\ \gramor & t / f & \textrm{Division}
\end{align*}
}

\gramlr{Facteur}{
\begin{align*}
f \gramisa & ( e )  & \textrm{Expression parenthésée}
\\ \gramor & f ** e & \textrm{Puissance}
\\ \gramor & n      & \textrm{Nombre}
\end{align*}
}

Cette notation définit les ensembles $E, T, F$ de la manière suivante :

…

% TODO cet exemple est syntaxique, c'est pas top
...
On définit ici quelques accesseurs. La notion d'accesseur utilisée ici est
directement inspirée des \emph{lentilles} utilisées en programmation
fonctionnelle, décrite dans~\cite{PierceLenses} et~\cite{LaarhovenLenses}.

\begin{definition}[Lentille]

Étant donnés deux ensembles $R$ et $A$, une \emph{lentille} $\mathcal{L} ∈
\textsc{Lens}_{R, A}$ (ou \emph{accesseur}) est consistuée des opérations
suivantes :

\begin{itemize}
\item
  une opération de lecture $\mathrm{get}_\mathcal{L} : R → A$
\item
  une opération de mise à jour $\mathrm{put}_\mathcal{L} : (A × R) → R$
\end{itemize}
...
\begin{align*}
\tag{GetPut}
\mathrm{put}_\mathcal{L}(a, \mathrm{get}_\mathcal{L}(r)) = & r \\
\tag{PutGet}
\mathrm{get}_\mathcal{L}(\mathrm{put}_\mathcal{L}(a, r)) = & a \\
\tag{PutPut}
\mathrm{put}_\mathcal{L}(a', \mathrm{put}_\mathcal{L}(a, r)) = & \mathrm{put}_\mathcal{L}(a, r) \\
\end{align*}

On note $\mathcal{L} =
\mkLens{\mathrm{get}_{\mathcal{L}}}{\mathrm{put}_{\mathcal{L}}}$.
...
\[
∀ i ∈ I, ∃ A_i, Δ(i) ∈ \textsc{Lens}_{R, A_i}
\]

On note alors

\begin{align*}
r [ i ]_{\mathcal{L}} \eqdef & \mathrm{get}_{Δ(i)}(r) \\
r [ i ← a ]_{\mathcal{L}} \eqdef & \mathrm{put}_{Δ(i)}(a, r) \\
\end{align*}
...
Soit $n ∈ ℕ$, et $n$ ensembles $E_1, …, E_n$.

Pour tout $i ∈ [1, n]$, on définit :

\begin{align*}
g_i((x_1, …, x_n)) = & x_i \\
p_i(y, (x_1, …, x_n)) = & (x_1, …, x_{i-1}, x_{i+1}, y, …, x_n)\\
\end{align*}

Définissons $T(i) = \mkLens{g_i}{p_i}$. Alors $T(i) ∈ \textsc{Lens}_{(E_1×…×E_n),E_i}$.
