...
Prenons l'exemple des listes chaînées composées d'élements de $ℕ$. On peut
définir cet ensemble $L$ de la manière inductive suivante :
...
\begin{align*}
  \opfix (f) & = \bigcup_{n=0}^{\infty} f^{n}(ø) \\
  f^0(x) & = x \\
  ∀ n > 0, f^n(x) & = f^{n-1}(f (x)) \\
\end{align*}
...
\begin{align*}
  a \gramisa & \textsc{Feuille}           & \textrm{Feuille}
  \\ \gramor & \textsc{Nœud}(a_1, n, a_2) & \textrm{Nœud}
\end{align*}
}
...
transition $\rightarrow$ entre états de l'interpréteur.
...
\begin{itemize}
\item notations champs/function/flottant
...
\item indices des accesseurs en majuscules
\end{itemize}
