...
\begin{figure}
  ...
  \gramlr{Phrases}{
    \begin{align*}
      p \gramisa & f                 & \textrm{Déclaration de fonction}
      ...
      \\ \gramor & x = e             & \textrm{Variable globale}
    \end{align*}
  }
...
%\begin{align*}
  %\mathrm{Cleanup}(s, g)  = & (s', g') \\
                    %où g' = & \mathrm{CleanupList}(g) \\
                       %s' = & [CleanupList(s_1), …, CleanupList (s_n)] \\
  %\mathrm{CleanupList}(u) = & \{(k, v) ∈ u / v n'est pas une adresse \} \\
                       %\cup & \{(k, φ) ∈ u / a n'est pas une adresse \} \\
  %\mathrm{Var}(a) = & a \\
  %\mathrm{Var}(*φ) = & ?
%\end{align*}

% TODO il faut casser les pointeurs, aussi. tri topo sur le graphe des chemins ?
...
\item accesseurs : utiliser des majs
