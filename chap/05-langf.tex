...
L'appel de fonction repose également sur cette évaluation. Tout d'abord, les
arguments sont évalués. Puis les expressions qui initialisent les variables
locales. Cf. figure~\ref{fcall-details}.
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \node (m0) {$m_0$};
    \node[below of=m0, node distance=2cm] (m1) {$m_1$};
    \node[right of=m1, node distance=3cm] (m2) {$m_2$};
    \node[below of=m2, node distance=2cm] (m3) {$m_3$};
...
\begin{itemize}
\item
  faire un premier push dans le call pour donner accès aux paramètres au
  locales?
\item
