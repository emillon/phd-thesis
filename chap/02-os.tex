Le système d'exploitation est le programme qui permet à un système
...
Un ordinateur est constitué de nombreux composants matériels : micro-processeur,
...
60, sont apparus les premiers systèmes ``à temps partagé'', permettant à
...
L'implatation d'un système d'exploitation est très proche du matériel sur
lequelle elle va s'exécuter. Pour étudier une implantation en particulier,
voyons ce que permet le matériel lui-même.
...
\begin{tikzpicture}
  ...
  \draw[dashed] (tlbl) -| (lbl.south);
  \draw[dashed] (tmov) -| (mov.south);
  \draw[dashed] (tops) -| (ops.south);
...
\todo{defs : intégrité, confidentialité}
...
page de 4 kibi-octets (figure~\ref{fig:pagetables}).
...
explication plus détaillée est disponible dans la documentation d'Intel
\cite{intelsys}.

\subsubsection{Dans la libc}
...
\subsubsection{Dans le handler d'interruption}
...
définit entre autres des points d'entrée pour les fonctionalités de débogage du
