...
le \emph{commit} \texttt{7786ce}), cette fonction appelle \texttt{memcpy} au
lieu de \texttt{copy\_from\_user} pour lire dans la mémoire du processus: en
théorie, si un utilisateur passe un pointeur noyau à la fonction
\texttt{ptrace}, il pourra lire des données du noyau.
...
est facile de voir comment repérer le bug: l'argument \texttt{addr} (typé
\textttdestiné à la base aux void *}) provenanespace utilisateur, est passé à \texttt{memcpy}. En annotant correctement les

Le-mini.diff est chargé de faire l'intermédiaire entre le matériel (processeur,
mémoire, périphériques, …) et les applications exécutées sur celui-ci (par
exemple un navigateur internet, une calculatrice ou un carnet d'adresses).
