...
% TODO uniformiser "appels systèmes" / "appels système"

\begin{figure}[h]
    \insertcode{entry-syscall.s}
    \caption{Point d'entrée des appels systèmes}
...
le \emph{commit} \texttt{7786ce} en remplaçant l'appel à \texttt{memcpy} par un
appel à \texttt{copy\_from\_user\_page}), cette fonction appelle \texttt{memcpy}
au lieu de \texttt{copy\_from\_user} pour lire dans la mémoire du processus. En
théorie, si un utilisateur passe un pointeur noyau à la fonction
\texttt{ptrace}, il pourra lire des données du noyau. L'appel
