...
autre processus, par exemple à des fins de débogage. Par exemple,
...
Comme pour la plupart des appels système, l'implantation de \texttt{ptrace} est
dépendante de l'architecture. Le deuxième exemple que nous présentons est celle
qui concerne les processeurs Blackfin, figure~\ref{fig:ptrace-blackfin}.
...
L'appel \texttt{ptrace(PTRACE\_PEEKDATA, p, addr)} permet ainsi non seulement
de lire les variables du processus \texttt{p} si \texttt{addr} est une
...
% TODO quoter le code qui appelle arch_ptrace avec la bonne task_struct
...
$(\ptrK{\tInt}, \ptrK{\tInt}, \tInt)$. Il est donc impossible de lui passer cet
argument.

% TODO polymorphisme
% TODO argument sizeof
