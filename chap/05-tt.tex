...
type, et non un schéma un de types. La seule exception notable est le pointeur
$\eNull$, qui a pour type $t~*$. Néanmoins cela ne repose pas sur une
généralisation de $t$, cela implique seulement que $\eNull$ est est compatible
avec n'importe quel type pointeur.
...
est soit une variable globale $(x)$ avec $x ∈ \cDom{Γ_G}$, soit une variable locale
$(|m|, x)$ du plus haut cadre de pile avec $x ∈ \cDom{Γ_L}$. Cela se prouve par
