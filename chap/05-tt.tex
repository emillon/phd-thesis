...
Si $Γ ⊢ v : t$ et $\mcomp{Γ}{m}$,
alors il existe $τ$ tel que
$m ⊧ v : τ$ et $\tComp{τ}{t}$.
...
On distingue selon la forme de $v$.

\begin{itemize}
\item $n$:
    Par inversion, $t = \tInt$. On pose $τ = \tInt$. On conclut avec
    \textsc{S-Int} et \textsc{Comp-Ground}.

\item $d$:
    Idem avec $τ = \tFloat$ et \textsc{S-Float}.

\item $\eUnit$:
    Idem avec $τ = \tUnit$ et \textsc{S-Unit}.

\item $\eNull$:
    Dans les hypothèses on a $Γ ⊢ \eNull : t$. D'après le lemme des formes
    canoniques, % TODO
    $∃ t', t = t'~*$.

    \def\cRepr#1{\mathrm{Repr}(#1)}

    Définissons un opérateur $\phx\cRepr$ qui à un type statique $t$ associe un
    type sémantique $τ$ tel que $\tComp{τ}{t}$:
    ...
    On pose alors $τ = \cRepr{t}$.
    Alors par \textsc{Comp-Ptr}, $\tComp{τ~*}{t~*}$.
    En outre, grâce à \textsc{S-Null}, on obtient $m ⊧ \eNull: τ~*$.

\item $\widehat{\&}~φ$:
    Par inversion, il existe $t'$ tel que
    $t = t'~*$. Puisque $\mcomp{Γ}{m}$,% TODO
    il existe $τ'$ tel que $m ⊧ m[φ]_Φ : τ'$ avec $\tComp{τ'}{t'}$.
    D'après \textsc{S-Ref}, $m ⊧ \widehat{\&}~φ : τ'~*$.
    En posant $τ = τ'~*$, on a donc $\tComp{τ}{t}$.

\item $\widehat{\eStruct{ l_1 : v_1 ; … ; l_n : v_n }}$:
    Par inversion, il existe $t_1, …, t_n$ tels que
    $t = \tStruct{l_1:t_1;…;l_n:t_n}$ et $∀i, Γ ⊢ v_i : t_i$.
    On applique l'hypothèse de récurrence à chacun des $v_i$:
    il existe $τ_i$ tel que $m ⊧ v_i : τ_i$ et
    $\tComp{τ_i}{t_i}$.

    Soit $τ = \tStruct{l_1:τ_1;…;l_n:τ_n}$. Par \textsc{S-Struct}, on a
    $m ⊧ \widehat{\eStruct{ l_1 : v_1 ; … ; l_n : v_n }} : τ$
    et par \textsc{Comp-Struct}, $\tComp{τ}{t}$.

\item $\widehat{ \eArray{v_1 ;…; v_n} }$:
    Par inversion, il existe $t'$ tel que
    $t = t'~[]$ et $∀i, Γ ⊢ v_i : t'$.

    En appliquant l'hypothèse de récurrence à chacun des $v_i$, il existe $τ'$
    tel que $∀i, m ⊧ v_i ; τ'$ avec $\tComp{τ'}{t}$.

    Soit $τ = τ'~[]$. Par \textsc{S-Array}, on a
    $m ⊧ \widehat{ \eArray{v_1, …, v_n}} : τ$,
    et par \textsc{Comp-Array}, $\tComp{τ}{t}$.

\item $\widehat{f}$:
    Posons $\widehat{f} = \widehat{\eFun{a_1,…,a_n}{i}}$. Par inversion, il
    existe $t_1, …, t_n, t'$ tels que $t = (t_1, …, t_n) → t'$.

    Soit alors $τ = \stFun{n}$. Par \textsc{S-Fun},
    $m ⊧ \widehat{f} : τ$ et
    et par \textsc{Comp-Fun}, $\tComp{τ}{t}$.
