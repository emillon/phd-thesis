...
\begin{figure}
  ...
  \gramlr{Structure}{
    \begin{align*}
      S \gramisa & \{ l_1 : t_1; … ; l_n : t_n \}  & \textrm{Structure simple}
    \end{align*}
  }
...
\begin{lemma}[Forme des left-values évaluées]
  \label{lemma:forme-lv}

  Si $\mmstar{m}{lv}{m'}{v}$, $v$ est de la forme $φ$ (référence mémoire) ou $Ω$
  (erreur).

  % TODO coinduction avec thm:progres ?
...
\begin{itemize}
\item virer la macro tstruct
\item virer la def de struct en phrase
