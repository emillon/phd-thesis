...
\item $t = t'*$:
    ...
    \begin{itemize}
        \item $v = \widehat{\&}~φ$:
            ...
            Puisque $\mcomp{Γ}{m}$ et $Φ ⊢ φ : t'$,% TODO
            $m[φ]_Φ$ est une valeur.

            Par induction, $m ⊧ m[φ]_Φ : τ'$ et $\tComp{τ'}{t'}$.
            D'après \textsc{S-Ref}, $m ⊧ \widehat{\&}~φ : τ$.

            De plus par \textsc{Comp-Ptr} % TODO nom pas cohérent ref/ptr
            $\tComp{τ}{t}$.
...
\item $\tStruct{ l_1 : t_1; … ; l_n : t_n }$: % TODO
    Soient $τ_i = \cRepr{t_i}$ et
    $τ = \tStruct{ l_1 : τ_1; … ; l_n : τ_n }$

    Par le lemme des formes canoniques, $v = \eStruct{l_1:v_1;…;l_n:v_n}$.

\item $t = (t_1, …, t_n) \rightarrow t$: % TODO
    Soit $τ = \stFun{n}$
