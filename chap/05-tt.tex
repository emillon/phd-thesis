...
Pour typer les structures, on suppose que les accès aux champs sont décorés du
...
ne sont pas présentes. On y utilise donc une variante du polymorphisme de
rangée~\cite{ocamlObjects} présent dans OCaml pour unifier deux types structures
partiellement connus. \added{référence}
...
Le fait d'effacer les types à l'exécution est un choix permettant
d'alléger les valeurs en mémoire. Il serait aussi possible de conserver les
types complets à l'exécution, afin de permettre une introspection dynamique des
valeurs, mais cela éloignerait le langage de C.
...
le type des valeurs à l'exécution présent dans $m$ est \enquote{compatible}
...
théorèmes. Les démonstrations des lemmes~\ref{lemma:inversion} et
\ref{lemma:canon} sont des analyses de cas laborieuses et sans difficulté; dans
ce cas on n'en donne que des esquisses.
...
\item Références mémoire:
  \begin{itemize}
    \item
      si $Γ ⊢ (x) : t$, alors $x : t ∈ Γ_G$
    \item
      si $Γ ⊢ (n, x) : t$, alors $x : t ∈ Γ_L$
    \item
...
\begin{proof}[Démonstration (esquisse)]
On procède comme pour le lemme d'inversion: pour chaque forme syntaxique, on
fait l'inventaire des règles pouvant arriver à cette dérivation.
\end{proof}
...
D'après la définition de $a$, $n ≤ |m|$.
...
\begin{itemize}
    \item Si $φ = φ'.l_S$, alors $\cVar{φ} = \cVar{φ'}$, et par inversion
        $Γ ⊢ φ' : S$. On conclut en appliquant l'hypothèse de récurrence à $φ'$.
    \item Si $φ = φ'[n]$, le cas est similaire.
    \item Si $φ = a$, alors $\cVar{φ} = a$. Si $a = (x)$, on a par inversion
        $x : t ∈ Γ_G$. Si $a = (n, x)$, alors par inversion $x : t ∈ Γ_L$.
        Il reste à montrer que $n = |m|$, ce qui peut se prouver par induction
        sur la dérivation de $\mcomp{Γ}{m}$, en notant que $\cDom{Γ_L}$ coincide
        toujours avec le dernier niveau de pile. Cette étape est ici admise.
\end{itemize}
\end{proof}
...
On suppose que $Γ ⊢ φ : t'$ et $m' = \cCleanVar{m - x}{x}$. Alors
$\extendLocal{Γ}{x}{t} ⊢ φ : t'$ par affaiblissement. On peut donc appliquer
l'hypothèse d'induction: $m[φ] = v$ où $m ⊧ v : τ'$ avec $\tComp{τ'}{t'}$. On
distingue alors selon la forme de $v$. Si $v = \wphi{φ'}$ où $\cVar{φ'} = (|m|,
x)$, alors $m'[φ] = \eNull$ par l'opération $\phxx\cCleanVar$. Le type $τ'$
étant un type pointeur par le lemme~\ref{lemma:canon}, on peut conclure. Dans
les autres cas $m[φ] = m'[φ]$ ce qui termine ce cas.
...
Soit $Γ$ un environnement de typage, $m$ un état mémoire et $p$ une phrase de
...
compilation la forme des valeurs produites par chaque expression. Pour ce faire,
