...
\begin{lemma}[Hauteur des chemins typés]
    ...
    Si $(s, g) ⊧ v : τ$, alors $\cH{v} ≤ |s|$. % TODO introduire |m|
...
\paragraph{$\widehat{c}$:} Alors $\cH{v} = -1$. Comme $|s| ≥ 0$, ce cas est
établi.
...
$m ⊧ v : τ$ est \textsc{S-Ref}, donc $m[(n, x)]_Φ$ est une valeur.
D'après la définition de $φ$, $n ≤ |s|$.

\paragraph{$\widehat{\&}~φ.l$:} %
On procède par induction sur $v' = \widehat{\&}~φ$.
Comme $\cH{\widehat{\&}~φ} ≤ |s|$ et
...
Par induction, $∀i ∈ [1;n], \cH{v_i} ≤ |s|$. Donc il en est de même pour leur
maximum, et $\cH{v} ≤ |s|$.
...
\begin{itemize}
\item
    Si $m = (s, g)$, alors chaque $(x ↦ v) ∈ m$, vérifie $\cH{v} ≤ |s|$
...
Soit $φ$ tel que $Γ ⊢ φ : t$. Soit $a = proj(φ)$. Par inversion,
% }}}
...
\item $t = (t_1, …, t_n) \rightarrow t'$:
    ...
    $m ⊧ v : τ$. On conclut d'autre par que $\tComp{τ}{t}$
