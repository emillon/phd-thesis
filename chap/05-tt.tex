% TODO global mettre _Φ implicite
...
$v = \wphi{φ'}$ où $\cVar{φ'} = (|m|, x)$, alors $m'[φ]_Φ = \eNull$ par
l'opération $\phxx\cCleanVar$. Le type $τ'$ étant un type pointeur par le
lemme~\ref{lemma:canon}, on peut conclure. Dans les autres cas $m[φ]_Φ =
m'[φ]_Φ$ ce qui termine ce cas.
...
\item Soit $\cVar{φ} = (x)$ avec $x ∈ \cDom{Γ_G}$, alors $Γ' ⊢ φ : t$ où $Γ' =
    ...
    récurrence en partant du jugement $Γ ⊧ m'$: il vient que $m'[φ]_Φ = v$ où $m
    ⊧ v : τ'$ avec $\tComp{τ'}{t'}$. Comme $\cH{v} ≤ |m'|$
    (lemme~\ref{lemma:hauteur-chem}), deux cas peuvent se produire:

\begin{itemize}
\item Si $\cH{v} = |m'|$, alors $m''[φ]_Φ = \eNull$ et on a bien la
    compatibilité mémoire (l'argument est similaire au cas \textsc{Decl-Clean}).

\item Sinon, $m''[φ]_Φ = m'[φ]_Φ$ et on conclut directement.
\end{itemize}
