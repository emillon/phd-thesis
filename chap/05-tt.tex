...
\cDom{Γ} &\eqdef \{ g_i / i ∈ [1;n] \} ∪ \{ l_i / i ∈ [1;m] \}\\ % TODO ou de Γ_*
...
\begin{proof}[Démonstration (esquisse)]
On fait l'inventaire, pour chaque forme syntaxique, des règles pouvant arriver à
cette dérivation.
\end{proof}
...
\def\cVars#1{\mathrm{Vars}(#1)} % TODO utile?

Commençons par donner un résultat préliminaire. On utilise les notations
suivantes:

\begin{itemize}

\item $\cVar{φ}$ la variable qui apparaît dans $φ$:
...
Si $\mcomp{Γ}{m}$ et $Γ ⊢ φ : t$, alors $\cVar{φ}$
est soit une variable globale $(x)$ avec $x ∈ Γ_G$, soit une variable locale
% TODO \cDom
$(|m|, x)$ du plus haut cadre de pile avec $x ∈ Γ_L$. Cela se prouve par
récurrence sur la dérivation de $\mcomp{Γ}{m}$.

% TODO présenter informellement l'autre

À l'aide de ce résultat, on prouve le lemme~\ref{lemma:mem-typ} par induction
sur une dérivation de $\mcomp{Γ}{m}$.
...
Alors la variable référencée par $φ$ est soit $(x)$, soit $(y)$ avec $y ∈ Γ_G$,
soit $(|m|, y)$ avec $y ∈ Γ_L$.

Dans le premier cas, $m'[φ]_Φ = v$, ce qui permet de conclure.

Dans les autres cas, $\cVar{φ} ∈ \cVars{m}$. Donc $m'[φ]_Φ = m[φ]_Φ$, ce qui
nous permet de conclure grâce à l'hypothèse d'induction.
% }}}
...
Si $\cVar{φ} = (x)$ avec $x ∈ \cDom{Γ_G}$, alors $Γ' ⊢ φ : t$ où
$Γ' = (Γ_G, [x_1:t_1, …, x_n:t_n, \vRet:t])$.
On applique alors l'hypothèse de récurrence sur $Γ ⊧ m'$: il vient que
$m'[φ]_Φ = v$ où $m ⊧ v : τ'$ avec $\tComp{τ'}{t'}$. Comme $\cH{v} ≤ |m'|$, deux
cas peuvent se produire. Si $\cH{v} = |m'|$, alors $m''[φ]_Φ = \eNull$ et on a
bien la compatibilité mémoire (l'argument est similaire au cas
\textsc{Decl-Clean}). Sinon, $m''[φ]_Φ = m'[φ]_Φ$ et on conclut directement.
...
Si $φ ≠ φ'$:
$Γ ⊢ φ' : t'$ donc $\cVar{φ'}$ est soit une locale soit une globale de $m'$. En
particulier $\cVar{φ'} ∈ \cVars{m'}$. Comme $φ ≠ φ'$, $m'[φ']_Φ = m[φ']_Φ$ et on
conclut grâce à l'hypothèse d'induction.
% }}}
\end{proof}
