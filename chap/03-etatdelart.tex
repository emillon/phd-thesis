...
ad-hoc nommé Sparse \link{sparse} peut être utilisé pour détecter les cas les
...
\item IA :
\begin{itemize}
\item citer pour penjili\cite{AllamigeonHymansSSTIC07}
\item difficultés :
...
\item
  Analyse dynamique \cite{oakland10} et Perl ?.
\item
  Analyse de flot :
...
cette technique à des langages compilés~\cite{clause-etal-issta07}.
...
and flow-sensitive properties such as locking invariants~\cite{pldi02}, similar
to the work on typestates~\cite{tse12-typestate}. It has even been applied to
...
Abstract interpretation~\cite{Cousot77} is a generic framework to analyze
