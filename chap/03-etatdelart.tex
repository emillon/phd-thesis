...
pour les exécutions de moins de $k$ étapes). Il est aussi possible de
partitionner l'espace d'états en classes
d'équivalence~\cite{partitionmodelchecking}, permettant de réduire le nombre
d'états de l'automate à explorer en ne considérant que des représentants de
chaque classe. Comme l'interprétation abstraite, ces analyses sont très
précises, au détriment d'un temps de calcul important à cause de l'explosion
combinatoire.

%TODO[S] le "partial order reduction" sert à réduire le nombre de classes
%d'équivalence à explorer, pas à rendre ça fini.
