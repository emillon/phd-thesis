...
l'approche que nous retiendrons.
...
En enrichissant le langage des types, on peut augmenter l'expressivité du typage
Par exemple, on peut former des types ``entier pair'', ``vecteur de $n$
...
vecteurs de $n$ entiers. C'est pourquoi on parle de types dépendants.
...
que les preuves sont à leur théorème. C'est l'isomorphisme de Curry-Howard.
...
B$, alors on peut en tirer une preuve de $B$.

\section{Conclusion}

% TODO[B] conclure + mini contributions (ce qu'on va ajouter, comment on se
% positionne)
