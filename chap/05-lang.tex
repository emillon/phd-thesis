...
La notion de bloc bien formé repose sur la portée des étiquettes : par exemple, une
instruction $\npkGoto{\cdot}$ est formée si son étiquette est dans l'ensemble
...
Un bloc $\npkDoWith{\cdot}{\cdot}$ est bien formé si son bloc interne est bien
formé; mais sous un ensemble d'étiquettes étendu du nouveau label, puisque ceci
y est visible.
...
Enfin, un $\npkGoto{\cdot}$ est bien formé sous un ensemble d'étiquettes
contenant sa destination.

\begin{mathpar}
\irule{WF-Goto}{lbl ∈ L}{L ‣ \npkGoto{lbl}}
\end{mathpar}

La définition de ce qu'est un bloc $b$ sans instruction $\npkReturn{\cdot}$
(noté $\NORET{b}$) est plus simple : c'est la définition récursive des blocs, en
omettant le cas de base $\npkReturn{\cdot}$.

\begin{mathpar}
     \irule{NR-Pass}{ }{ \NORET{ε} }
\and \irule{NR-Seq}{\NORET{i} \\ \NORET{b}}{ \NORET{i;b} }
\and \irule{NR-Set}{ }{ \NORET{lv \leftarrow e}}
\and \irule{NR-Fcall}{ }{lv \leftarrow funexp (args)}
\and \irule{NR-While}{\NORET{b}}{\npkWhile{b}}
\and \irule{NR-Decl}{ \NORET{b} }{\npkDecl{nom}{b}}
