...
\begin{tikzpicture}
  [cfgnode/.style={draw,shape=circle}]
  \node[cfgnode] (l) {$l$};
  \node[cfgnode,right of=l, node distance=2.5cm] (lp) {$l'$};
  \node[cfgnode,below of=l, xshift=5mm] (lb) {$l_b$};
  \node[cfgnode,below of=lp, xshift=-5mm] (lbp) {$l_b'$};
  \draw[->] (l)  to node[auto] {$↑x\{b\}$} (lp);
  \draw[->] (lb) to node[auto] {$b$} (lbp);
  \draw[dashed] (l) to (lb);
  \draw[dashed] (lp) to (lbp);
\end{tikzpicture}
\end{minipage}
...
\begin{tikzpicture}
  ...
  \draw[->] (l)  to node[auto] {$if(e)\{i_t\} else \{i_f\}$} (lp);
