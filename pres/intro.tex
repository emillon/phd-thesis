...
\long\def\mec{
    \draw[ultra thick, fill=MellonGreen]
              (0, 0)
        -- ++ (1, -2)
        -- ++ (1, 0)
        -- ++ (-1, 3)
...
\begin{frame}{L'arnaque du serrurier}
    ...
    % TODO serrurier + porte fermée = porte ouverte
...
\begin{frame}{Types de copies}
    ...
    \begin{tikzpicture}
        ...
        \draw[fill=MellonGreen!50] (0, 0) rectangle (-6, 6);
        \draw[fill=MellonPink!50] (0, 0) rectangle (2, 6);

        \draw[->] (-1, 5)  to[bend left]  node[lbl] {\texttt{copy\_from\_user}} ++ (2, 0);
        \draw[->] (-1, 4)  to[bend left]  node[lbl] {\sout{\texttt{memcpy}}} ++ (2, 0);
        \draw[<-] (-1, 2)  to[bend right] node[lbl,swap] {\texttt{copy\_to\_user}} ++ (2, 0);
        \draw[<-] (-1, 1)  to[bend right] node[lbl,swap] {\sout{\texttt{memcpy}}} ++ (2, 0);
